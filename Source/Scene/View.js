define([
    ...
    function View(scene, camera, viewport) {
        ...
        this.camera = camera;
        this.cameraClone = Camera.clone(camera);
        this.cameraStartFired = false;
        this.cameraMovedTime = undefined;
    ...
    View.prototype.checkForCameraUpdates = function(scene) {
        ...
        var cameraClone = this.cameraClone;
        if (!cameraEqual(camera, cameraClone, CesiumMath.EPSILON15)) {
            if (!this.cameraStartFired) {
                camera.moveStart.raiseEvent();
                this.cameraStartFired = true;
            }
            this.cameraMovedTime = getTimestamp();
            Camera.clone(camera, cameraClone);

            return true;
        }

        if (this.cameraStartFired && getTimestamp() - this.cameraMovedTime > scene.cameraEventWaitTime) {
    ...
    View.prototype.createPotentiallyVisibleSet = function(scene) {
        ...
        if (frameState.useLogDepthDirty || (near !== Number.MAX_VALUE && (numFrustums !== numberOfFrustums || (frustumCommandsList.length !== 0 &&
